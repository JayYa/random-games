...
**主题就是一份名单文件。** 往 `public/` 扔一份 CSV，在头部写一行 `# entry:`，站点上就多一个主题——不动任何代码（[ADR-0009](docs/adr/0009-themes-are-discovered-from-csv-files.md)）。

最小的一份主题长这样，存成 `public/drink.csv` 就能用：
...
主题面向使用者的说法写在同一份文件的注释行里，三个键，半角冒号，冒号后的首尾空白会被裁掉：

| 键 | 写在哪 | 必填 | 缺了怎么办 |
| --- | --- | --- | --- |
| `entry` | 选主题页上这个入口的说法 | 是 | 整份文件被跳过 |
| `title` | 玩法页上的大标题 | 否 | 退回 `entry` |
| `result` | 结果卡片上中选名字前面那句话 | 否 | 退回中性的「今天就来」 |

`entry` 必填，是因为入口的说法是唯一没法从文件名猜出来的东西——总不能在选主题页上写个 `drink`。三个键都写全就是这样：
...
只有这三个键有特殊含义，其余 `#` 行一律当散文跳过，所以名单头部那段给人读的说明不会被当成配置。

**文件名就是地址里的那一段**：`drink.csv` 对应 `#/drink`，发给同事的链接长什么样，写文件名的时候就知道了。文件主名只能用小写字母、数字和连字符（`[a-z0-9-]+`），大写字母、中文、空格都不行——不然地址栏里会是一串百分号转义。

**写坏了只连累它自己**：文件名不合规、或者漏了 `# entry:`，那一份被跳过并打一行日志（本地 `pnpm dev` 时也打），构建照常成功，其余主题照常上线。真正拦住"写坏了却没人发现"的是 `pnpm test` 里的一条测试，它会遍历 `public/*.csv` 断言没有任何一份被跳过。

主题清单是构建期扫 `public/*.csv` 得出的，编译进产物，浏览器里不会因此多一次请求。想知道站上有哪些主题，看 `public/` 下有哪些 CSV。
