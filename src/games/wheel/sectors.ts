 ...
 *
 * 这条不变量——转盘停下之后，指针 (Pointer) 底下的扇区就是先定的那一个——原本被拆成
 * 三段散文写在会话、动画、画布三个模块的注释里，三处各用一套坐标说法，
 * 谁都不负责它。三段话必须同时为真转盘才停得对，而反算算错了肉眼极难发现
...
export interface Sectors {
  /** 扇区数。转盘上恒为 12（见 `./session.ts` 的 `SECTOR_COUNT`），与名单大小无关。 */
  readonly count: number;
  /**
   * 扇区 i 内的落点角度（转盘自身坐标，`[0, 2π)`）。
   * `r` 是 `[0, 1)` 的随机数，只决定停在这一格的什么位置，不决定停在哪一格。
   */
 ...
 * 落在哪条、带子里的哪一点，都还是均匀的——停在哪一格已经定完了，这个映射
 * 只决定「停在这一格的什么位置」，不影响停在哪一格（故事 12）。
 *
 * 两头各留 10% 的边距：指针有实际宽度，落点贴着扇区边界时，
 * 肉眼会觉得指针正卡在两个扇区中间，说不清名字该写进哪一格（故事 4）。
...
/**
 * `count` 是扇区数，一次挂载里定死，所以只在这里出现一次，
 * 不必再跟着每个成员的入参跑一遍。
 *
 * `count = 0` 在造工厂的这一刻就抛：没有扇区的转盘转不动，这是不该发生的
 * 情况，静静吞掉只会变成一个转不起来的盘面。
 */
